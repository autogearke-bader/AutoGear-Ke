...
try {
    $stmt = $pdo->query("SELECT p.id, p.slug, p.name, p.price, p.description, p.category, p.stock_status, p.has_installation, p.is_new, p.video_url, p.youtube_url, p.instagram_url, p.tiktok_url, p.use_white_bg, GROUP_CONCAT(CONCAT_WS('|||', pi.image_name, IFNULL(pi.alt_text, '')) SEPARATOR ';;;') as image_data FROM products p LEFT JOIN product_images pi ON p.id = pi.product_id GROUP BY p.id");
