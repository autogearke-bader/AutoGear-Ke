<?php
/**
 * Add Article Endpoint - Secure Version
...
error_log("add-article.php called - " . $_SERVER['REQUEST_METHOD']);
...
error_log("add-article.php received data: " . json_encode($data));
...
$slug = generateUniqueArticleSlug($pdo, $baseSlug);

try {
    $stmt = $pdo->prepare("
        INSERT INTO articles (
            slug, title, featured_image, content, excerpt,
            meta_description, seo_keywords, category, brand,
            related_products, related_bundles, is_published, 
            is_featured_homepage, is_featured_car, is_featured_mobile
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ");
    ...
    $stmt->execute([
        ...
        $relatedProducts, $relatedBundles, $isPublished, 
        $isFeaturedHomepage, $isFeaturedCar, $isFeaturedMobile
    ]);
    
    $articleId = $pdo->lastInsertId();
    
    // Insert article-brand relationships
    if (!empty($brandIds)) {
        foreach ($brandIds as $bid) {
            $stmt = $pdo->prepare("INSERT IGNORE INTO article_brands (article_id, brand_id) VALUES (?, ?)");
            $stmt->execute([$articleId, $bid]);
        }
    }
    
    log_security_event('article_created', 'Article created successfully', [
...
if (!$articleId || $articleId < 1) {
    error_log('add-article.php ERROR: Invalid ID returned from lastInsertId: ' . var_export($articleId, true));
    http_response_code(500);
    echo json_encode([
        'success' => false,
        'error' => 'Failed to create article - invalid ID generated'
    ]);
    exit;
}

error_log('add-article.php SUCCESS: Created article with ID ' . $articleId);
...
if (!empty($featuredImage)) {
    ...
    if (!$imageResult['success']) {
        error_log('Article image upload failed: ' . $imageResult['error']);
    } else {
        ...
        if (!$uploadResult['success']) {
            error_log('Failed to store article image metadata: ' . $uploadResult['error']);
        } else {
            // Update article with the actual image filename
            $stmt = $pdo->prepare("UPDATE articles SET featured_image = ? WHERE id = ?");
            $stmt->execute([$imageFilename, $articleId]);
        }
    }
}
...
echo json_encode([
    'success' => true,
    'id' => (int)$articleId,  // Ensure it's an integer
    'slug' => $slug,
    'category' => [
        'id' => $categoryId,
        'name' => $category,
    ...
    error_log("add-article.php PDOException: " . $e->getMessage());
    http_response_code(500);
    echo json_encode(['error' => 'Failed to create article: ' . $e->getMessage()]);
}


function generateUniqueArticleSlug($pdo, $baseSlug) {
    $slug = $baseSlug;
    $counter = 1;
    $originalSlug = $slug;
