...
const GadgetsPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Set Mobile Accessories SEO values
    document.title = `Mobile Phone Accessories Kenya | Power Banks & Chargers`;

    if (metaDescription) {
      metaDescription.setAttribute('content', 'Find high-quality mobile accessories at AutoGear Ke. Fast charging stations, durable phone covers, smartwatches & portable speakers. Shop online in Nairobi.');
    }

    if (metaKeywords) {
      metaKeywords.setAttribute('content', 'mobile accessories nairobi, phone holders kenya, fast chargers nairobi, iphone accessories kenya, samsung chargers nairobi, car phone mounts kenya, tech gadgets nairobi, mobile gear kenya, type-c cables nairobi');
    }

    if (ogTitle) ogTitle.setAttribute('content', `Mobile Accessories in Nairobi | ${BUSINESS_NAME}`);
    if (ogDescription) ogDescription.setAttribute('content', 'Shop durable phone accessories with nationwide delivery. Holders, chargers, cables, and more. Reliable tech essentials from Nairobi.');
    if (ogUrl) ogUrl.setAttribute('content', 'https://autogearke.com/mobile-accessories');
    if (canonicalLink) canonicalLink.setAttribute('href', 'https://autogearke.com/mobile-accessories');
    ...
    const fetchData = async () => {
      try {
        const [productsRes, bundlesRes, articlesRes] = await Promise.all([
          ...
          fetch(`${API_BASE_URL}/get-featured-mobile.php?limit=1`)
        ]);
        if (productsRes.ok) {
          const allProducts = await productsRes.json();
          const gadgetProducts = allProducts.filter((p: Product) => p.category === 'gadget');
          setProducts(gadgetProducts);
        }
        if (bundlesRes.ok) {
          const bundlesData = await bundlesRes.json();
          setBundles(bundlesData.filter((b: Bundle) => b.category === 'gadget' || !b.category));
        }
        if (articlesRes.ok) {
  ...
  const customInquiryMessage = "Hi, I'm looking for a specific mobile accessory not on your list. Can you help me find it?";
  const customInquiryUrl = `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(customInquiryMessage)}`;

  return (
    <div className="pb-0">
      <Helmet>
        <title>Mobile Accessories | AutoGear Ke - Phones & Gadgets Nairobi</title>
        <link rel="canonical" href="https://autogearke.com/mobile-accessories" />
        <meta name="description" content="Shop mobile accessories in Nairobi. Fast chargers, wireless earbuds, phone cases, smartwatches. Quality verified with pay on delivery." />
      </Helmet>
        ...
        {JSON.stringify({
          ...
          "itemListElement": [
            ...
            },
            {
              ...
              "item": "https://autogearke.com/mobile-accessories/"
            }
          ]
        })}
      </script>
      ...
      <section className="px-6 pt-2 lg:pt-2 pb-6 max-w-7xl mx-auto">
        <div className="mb-2 md:mb-4">
          <h2 className="text-xl md:text-2xl font-black text-white uppercase mb-2">Shop Mobile & Gadget Accessories</h2>
          <p className="text-slate-400 text-sm">Explore our range of mobile & gadget accessories.</p>
        </div>
        {products.length > 0 ? (
          ...
          <div className="text-center py-20 border-2 border-dashed border-slate-800 rounded-3xl">
            <p className="text-slate-500 text-sm">No mobile accessories found. Please check back later.</p>
          </div>
        )}
      </section>
      ...
      <section className="px-6 pb-12 md:pb-20 bg-slate-100/50 dark:bg-slate-900/50 border-y border-slate-200 dark:border-slate-900">
        <div className="max-w-7xl mx-auto">
          <div className="mb-8 text-center md:text-left flex items-center justify-between">
            <div>
              <h2 className="text-1xl md:text-2xl font-black text-slate-900 dark:text-white uppercase mb-2">Exclusive Bundles</h2>
              <p className="text-slate-500 dark:text-slate-400 text-sm">Best value mobile accessory kits in Kenya.</p>
            </div>
      ...
      {/* Pro-Guides Section - Tech & Charging Tips */}
      {featuredArticles.length > 0 && (
        <section className="px-6 py-2 md:py-3 max-w-7xl mx-auto">
          <div className="mb-6">
            <h2 className="text-sm md:text-sm font-black text-slate-900 dark:text-white uppercase">Tech & Charging Tips</h2>
          </div>
        ...
        <div className="max-w-4xl mx-auto rounded-xl md:rounded-[2rem] bg-slate-900 border border-slate-800 p-4 md:p-8 shadow-2xl relative overflow-hidden group">
          <div className="relative z-10 flex flex-row items-center justify-between gap-4 md:block md:text-center">
            ...
            <p className="hidden md:block text-slate-400 text-sm md:mb-6 leading-relaxed max-w-2xl mx-auto">
              If you don't see the exact mobile accessory you're looking for, message us. We have access to extensive warehouse stock.
            </p>
